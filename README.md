...
华师在线课程网站平台eCourse是一个基于`Node.js`和`MongoDB`搭建的网站，为教师和学生提供课程回顾、沟通交流、答疑解惑、资料共享、作业提交的功能。
...
3. Express框架
    ...
    1. 路由控制
        ...
        Express支持REST（Representational State Transfer，表征状态转移）风格的请求方式，它是一种基于HTTP协议的网络应用的接口风格，充分利用HTTP的方法实现统一风格接口的服务。
    2. 模板引擎
    ...
    3. 中间件
        ...
        Express是一个由路由和中间件构成的一个web开发框架，从本质上说一个Express应用就是在调用各种中间件，中间件的功能包括执行代码、修改请求和相应对象、终结请求-响应循环、调用堆栈中的下一个中间件等。如果当前中间件没有终结请求-响应循环，则必须调用next()方法将控制权交给下一个中间件，否则请求就会挂起。
...
## 需求分析
2.1 问题背景及描述
    课后学习是大学学习中很重要的一部分。在课堂上，由于时间所限，老师可能有一些遗漏的地方，而学生当场往往无法很好地理会老师的意思，在课后又苦于没有合适的平台来进行课后复习与资料下载，学生和老师之间缺乏沟通交流、答疑解惑、共享资料、提交作业的平台。从这一需求出发，结合新兴的Node.js技术，为华师师生提供一个课程网站平台eCourse。
    现将课程网站平台的业务流程抽象如下：
    1. 教师在课程网站平台上输入课程类型、课程名和课程简介发表自己的课程，等待系统管理员审核该课程，该课程完成审核前无法使用其他功能，包括加入、发表评论、添加课程内容等。
    2. 系统管理员审核该课程类型、课程名和简介，无误后确认审核成功，教师收到平台提醒，此时该课程可以正常使用所有功能。
    3. 教师添加课程内容和上传课件。
    4. 学生加入或退出课程，在课程下发表自己的评论或回复其他评论，浏览课程内容并下载课件、提交作业。
    5. 教师在课程内容页看到学生提交的作业，并进行打分和批复。
    6. 教师修改课程简介，修改后需系统管理员重新审核。
    7. 教师修改课程内容和删除课件，或删除课程内容。
    8. 在用户个人主页可以看到该用户发表或加入的课程、关注该用户，在已登陆用户的主页中可以修改自己的头像和个人资料、密码，查看已登陆用户的被回复情况和作业批复情况。
    
2.2 功能分析
    1. 用户注册与登陆：用户注册需填写学号（用户名）、密码、身份（教师/学生）、院系、邮箱地址、个人简介并上传头像，登陆时使用用户名和密码登陆。
    2. 发表、删除课程：教师用户填写课程类型、课程名和课程简介后发表课程，或删除自己发表的课程。
    3. 审核课程：系统管理员审核课程。
    4. 回复、删除评论：用户在课程页输入评论内容后发表评论或选中某条评论进行回复，或删除自己的评论。
    5. 添加、删除课时：课程作者（教师）填写课时、标题和内容后添加课时，或删除自己发布的课时。
    6. 上传、删除课件：课程作者（教师）并上传课件，格式限制为ppt、doc、pdf、txt、rar或zip，大小限制为10M，或删除自己上传的课件。
    7. 加入、退出课程：系统检测学生是否已加入该课程，并给出相应的加入/退出课程按钮。
    8. 提交、删除作业：学生在课程内容页提交该课时的作业，格式限制为ppt、doc、pdf、txt、rar或zip，大小限制为10M，或删除自己提交的作业。
    9. 添加、删除作业批复：教师在课时页看到所有学生提交的该课时的作业，并选中某作业进行下载，或输入批复内容进行批复。
    10.
...
4. 模型设计
    ...
    7. 课程作业
        ...
        ```
    8. 作业批复
        作业批复文档存储课程内容id、课程id、作者id和批复内容字段。
        ```
...
1. 系统结构
    ...
    |   |-- welcome.js      首页重定向至localhost:3000/courses
    |   |-- index.js        总配置，分发路由
...
8. 后台验证
    由于前端验证可以轻易被绕开，所以本系统采用后台验证方式，在接受用户请求后都会进行参数验证，如参数不合法将抛出异常，并进行页面通知。
